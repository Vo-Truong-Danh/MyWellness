...
class _HealthHistoryScreenState extends State<HealthHistoryScreen> with WidgetsBindingObserver {
  ...
  Future<void> _loadHealthHistory() async {
    ...
    try {
      ...
      if (user != null) {
        final QuerySnapshot healthData = await FirebaseFirestore.instance
            .collection('users')
            .doc(user.uid)
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Không thể tải lịch sử sức khỏe. Vui lòng thử lại!')),
        );
      }
      setState(() {
        _isLoading = false;
  ...
  Widget _buildHealthRecordItem(Map<String, dynamic> record) {
    ...
    final bloodPressure = record['bloodPressure'] != null
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildHealthDetail('Nhịp tim', '$heartRate bpm', Icons.favorite_border),
            _buildHealthDetail('Số bước chân', stepCount, Icons.directions_walk),
            _buildHealthDetail('Giờ ngủ', '$sleepHours giờ', Icons.bedtime),
            _buildHealthDetail('Nước uống', '$waterIntake ml', Icons.water_drop),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _healthRecords.isNotEmpty
