...
class HealthDataProvider extends ChangeNotifier {
  ...
  Future<void> loadDailyLog(DateTime date) async {
    ...
    try {
      ...
      print(
        "Đã tải nhật ký cho ngày: ${DateFormat('yyyy-MM-dd').format(date)}",
      );
      if (_dailyLog != null) {
        print(
          "Đã tìm thấy dữ liệu: ${_dailyLog!.nutritionLogs?.length ?? 0} món ăn, ${_dailyLog!.workoutLogs?.length ?? 0} bài tập",
        );
      } else {
  ...
  double? getLatestHeartRate() {
    if (_dailyLog?.heartRateReadings == null ||
        _dailyLog!.heartRateReadings!.isEmpty) {
      return null;
    }
  ...
  BloodPressureEntry? getLatestBloodPressure() {
    if (_dailyLog?.bloodPressureReadings == null ||
        _dailyLog!.bloodPressureReadings!.isEmpty) {
      return null;
    }
    // Sắp xếp theo thời gian giảm dần và lấy giá trị đầu tiên
  ...
  Future<bool> addHeartRateReading(double value, DateTime date) async {
    ...
    if (success) {
      ...
      if (_dailyLog != null) {
        // Tạo bản ghi mới
        HeartRateEntry newEntry = HeartRateEntry(
          value: value,
          time: DateTime.now(),
        );
  ...
  Future<void> deleteWorkout(WorkoutEntry workout) async {
    if (_dailyLog != null && _dailyLog!.workoutLogs != null) {
      final index = _dailyLog!.workoutLogs!.indexWhere(
        (w) => w.id == workout.id,
      );
      if (index != -1) {
  ...
  Future<void> deleteNutrition(FoodEntry nutrition) async {
    if (_dailyLog != null && _dailyLog!.nutritionLogs != null) {
      final index = _dailyLog!.nutritionLogs!.indexWhere(
        (n) => n.id == nutrition.id,
      );
      if (index != -1) {
        // Lưu lại để hoàn tác nếu cần
  ...
  void undoDeleteNutrition() {
    if (_lastDeletedNutrition != null &&
      ...
      double newTotalCalories =
          (_dailyLog!.loggedCalories ?? 0) + _lastDeletedNutrition!.calories;
  ...
  double getCalorieProgress() {
    if (_userData?.dailyCalorieTarget == null ||
        _userData!.dailyCalorieTarget == 0) {
      return 0.0;
    }
  ...
  double getDailyGoalProgress() {
    ...
    if (_userData?.dailyWaterTarget != null &&
        _userData!.dailyWaterTarget! > 0) {
      waterProgress =
          (_dailyLog?.currentWaterIntake ?? 0) / _userData!.dailyWaterTarget!;
    }

    double calorieProgress = getCalorieProgress();

    double stepsProgress = 0.0;
    if (_userData?.dailyStepsTarget != null &&
        _userData!.dailyStepsTarget! > 0) {
      stepsProgress =
          (_dailyLog?.currentSteps ?? 0) / _userData!.dailyStepsTarget!;
    }
